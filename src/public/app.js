...
function renderPlayers(payload, maxPlayers) {
...
$('#shutdown-button').addEventListener('click', async () => {
  ...
  const message = window.prompt('Optional shutdown message:', 'Server shutting down soon') ?? '';
  ...
  if (window.confirm(`Shut down the Palworld server in ${displayedMinutes} minute${displayedMinutes === 1 ? '' : 's'}?`)) {
    await action('shutdown', { message, waittime });
  }
});
