...
function isDemoAction(action, config, endpoints) {
  return config.demoMode && endpoints[action]?.method === 'POST';
}
...
export function createRequestHandler({ config, endpoints, demoStatus, publicDir, client, hostMetrics, processManager, notifications, scheduler, shutdownController }) {
    ...
    try {
      ...
      if (url.pathname === '/api/action' && request.method === 'POST') {
        ...
        if (action === 'shutdown') {
          ...
          const data = await client.call(endpoints[action], { ...payload, waittime });
